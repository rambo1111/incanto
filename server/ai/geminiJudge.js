// server/ai/geminiJudge.js — Gemini 2.5 Flash vision judge with 3-tier fallback
...
const VALID_TYPES = [
  'p1_loses','p2_loses','both_lose','none_lose',
  'p1_inappropriate','p2_inappropriate',
];
...
Look at both drawings carefully, consider the spell names, then judge the clash. Return ONLY the JSON.`;
    ...
    if (!VALID_TYPES.includes(parsed.type)) throw new Error(`Bad type: ${parsed.type}`);

    console.log(`✅ Gemini judged: ${parsed.type} — "${parsed.reason}"`);
    return parsed;

  } catch (err) {
    console.error('⚠️  Vision judge error:', err.message);
    return judgeByNamesOnly(player1, player2);
  }
}

/** Fallback: text-only judge — spell names only */
...
Respond with ONLY a JSON object (no markdown, no backticks):
{
  ...
  "narrative": "<2 vivid sentences about what happened when these specific spells clashed>"
    ...
    const safe = ['p1_loses','p2_loses','both_lose','none_lose'];
    if (!safe.includes(parsed.type)) throw new Error(`Bad type: ${parsed.type}`);

    console.log(`✅ Text fallback judged: ${parsed.type}`);
    return parsed;

  } catch (err) {
    console.error('⚠️  Text fallback error:', err.message);
...
function randomFallback(player1, player2) {
  ...
  const narratives = {
    p1_loses:  `${player1.username}'s "${player1.spellName}" shattered against ${player2.username}'s overwhelming "${player2.spellName}", crumbling into sparks. The arena fell silent as ${player1.username} staggered back.`,
    ...
    both_lose: `"${player1.spellName}" and "${player2.spellName}" collided in a catastrophic feedback loop. Both ${player1.username} and ${player2.username} were hurled back by the mutual destruction.`,
    none_lose: `"${player1.spellName}" and "${player2.spellName}" met in perfect equilibrium, their energies spiraling into a harmless vortex. A rare and humbling stalemate.`,
  };
  const reasons = {
  ...
  console.log(`🎲 Random fallback: ${type}`);
