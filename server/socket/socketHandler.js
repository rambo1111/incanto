...
// Grace-period timers: 1-second window after draw timer ends for in-flight submissions
...
function setupSocketHandlers(io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_room', ({ username, roomCode }) => {
      ...
      if (result.room.players.length === 2) startNextRound(io, code);
    });
    ...
    socket.on('join_random', ({ username }) => {
      ...
      if (existingRoom) {
        ...
        if (!result.error) {
          ...
          if (result.room.players.length === 2) startNextRound(io, existingRoom.code);
        } else {
    ...
    socket.on('submit_spell', ({ roomCode, imageData, spellName }) => {
      ...
      // Prevent double-submission from same socket
      ...
      if (subCount === 2) {
        // Both submitted — cancel draw timer + grace window, then judge immediately
        stopTimer(code);
    ...
    socket.on('disconnect', () => {
      ...
      if (!result.roomDeleted && result.room) {
        ...
        if (['drawing', 'judging', 'round_result'].includes(result.room.state)) {
          io.to(remaining.socketId).emit('game_over', {
            winner: remaining.username,
            players: [{ username: remaining.username, lives: remaining.lives, isYou: true }],
  ...
  function startNextRound(io, roomCode) {
    ...
    console.log(`⚔️  Round ${room.round}/${MAX_ROUNDS} — room ${roomCode}`);
    ...
    startTimer(
      ...
      () => {
        // Grace period: 1 second for in-flight submissions
        const t = setTimeout(() => {
  ...
  async function processRound(io, roomCode) {
    ...
    const spell1 = { username: p1.username, spellName: sub1.spellName || 'An empty canvas', imageData: sub1.imageData || whiteCanvas() };
    const spell2 = { username: p2.username, spellName: sub2.spellName || 'An empty canvas', imageData: sub2.imageData || whiteCanvas() };

    const outcome = await judgeRound(spell1, spell2);

    // Re-fetch after async — players may have disconnected during judging
    const freshRoom = getRoom(roomCode);
    if (!freshRoom) {
      console.log(`⚠️  Room ${roomCode} gone after judging`);
      return;
    }
...
/** Solid-white 1×1 PNG for missing submissions */
